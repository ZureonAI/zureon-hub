    ...
    <ScreenLayout>
      ...
      {wallet.connected && (
        <section className="flex flex-col gap-md">
          ...
          {!txLoading && txs.length > 0 && (
            <div className="flex flex-col gap-md">
              {/* Dashboard shows only a short preview of the newest activity
                  across all assets; the full per-token history lives on each
                  asset detail screen (progressive disclosure). */}
              {txs.slice(0, 4).map(tx => <TxRow key={tx.event_id} tx={tx} walletAddress={wallet.address ?? ''} />)}
            </div>
          )}
        </section>
      )}
